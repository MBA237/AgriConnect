...
import { createProduct, updateProduct, getProduct, getRegisteredSellers, type RegisteredSeller } from '../../services/api'
...
export default function CreateOffer() {
  ...
  useEffect(() => {
    if (!id && session.user?.name) {
      setForm(prev => ({ ...prev, farmerName: session.user.name }))
    }
  ...
  useEffect(() => {
    ...
    const loadSellers = async () => {
      try {
        const response = await getRegisteredSellers()
        if (active) setSellers(response.data.sellers)
      } catch {
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.price || !Number.isFinite(Number(form.price)) || Number(form.price) <= 0) newErrors.price = 'Le prix doit être supérieur à 0'
    if (!form.stock || !Number.isInteger(Number(form.stock)) || Number(form.stock) <= 0) newErrors.stock = 'Le stock doit être un entier supérieur à 0'
  ...
  const canPublish = session.user?.role === 'agriculteur' || (session.user?.role as string) === 'FARMER'
  ...
  if (loading) return <div className="offer-loading" role="status"><span className="spinner" aria-hidden="true"></span><span>Chargement de l'offre...</span></div>
  ...
  return (
    <section className="page offer-page">
      ...
      <div className="card offer-form-card">
        <form className="offer-form" onSubmit={handleSubmit}>
          {Object.keys(errors).length > 0 && (
            <div className="alert alert-error offer-form-alert" role="alert">
              <div className="alert-body">
                <div className="alert-title">Vérifiez votre offre</div>
                <div className="alert-message">Certains champs obligatoires doivent être corrigés avant publication.</div>
              </div>
            </div>
          )}
          ...
          <div className="grid-2">
            ...
            <div className="input-group">
              ...
              />
              {errors.deliveryTime && <span className="field-error" role="alert">{errors.deliveryTime}</span>}
            </div>
          </div>

          <div className="input-group">
            <label htmlFor="farmerName">Rechercher un vendeur enregistré</label>
            <input 
              id="farmerName"
              name="farmerName" 
              ...
              aria-describedby="farmerName-help"
            />
            <datalist id="registered-sellers">
              {sellers.map(seller => <option key={seller.id} value={seller.fullName || seller.email || seller.id}>{seller.email}</option>)}
            </datalist>
            <span id="farmerName-help" className="field-help">Les noms proposés proviennent des vendeurs enregistrés. La publication reste liée à votre compte connecté.</span>
          </div>
          ...
          <div className="offer-form-actions">
            <button className="btn-primary offer-submit" type="submit" disabled={saving} aria-busy={saving}>
              {saving ? <><span className="spinner spinner-light" aria-hidden="true"></span>{id ? 'Mise à jour...' : 'Publication...'}</> : <><i className="fas fa-paper-plane" aria-hidden="true"></i>{id ? 'Mettre à jour' : 'Publier l\'offre'}</>}
            </button>
            <button className="btn-outline offer-cancel" type="button" onClick={() => navigate('/catalog')} disabled={saving}>Annuler</button>
          </div>
        </form>
      </div>
    </section>
